...
describe('StarToken', () => {
  ...
  beforeEach(async () => {
    ...
    starToken = <StarToken>await factory.deploy(await owner.getAddress(), initialSupply);
  ...
  describe('mint', () => {
    ...
    it('should assign minted tokens to the correct wallet address', async () => {
      ...
      const amount = parseEther('1')
      await starToken.mint(ownerAddress, amount);
      expect(await starToken.balanceOf(ownerAddress)).to.eq(initialSupply.add(amount))
    });
    ...
    it('should not allow unauthorized addresses to mint tokens', async () => {
      ...
      await expect(starToken.connect(signers[1]).mint(unauthorized, amount))
        .to.be.reverted
    });
  });
  ...
  describe('burn', () => {
    ...
    it('should burn tokens from the correct wallet address', async () => {
      ...
      const ownerAddress = await owner.getAddress();
      await starToken.burn(ownerAddress, amount);
      expect(await starToken.balanceOf(ownerAddress)).to.eq(initialSupply.sub(amount))
    });
    ...
    it('should not allow unauthorized addresses invocations', async () => {
      const amount = parseEther('1')
      const unauthorized = await signers[1].getAddress();
      await expect(starToken.connect(signers[1]).burn(unauthorized, amount))
        .to.be.reverted
    });
  });
});